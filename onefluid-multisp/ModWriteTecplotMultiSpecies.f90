...
contains
  ...
  subroutine write_tecplot_auxdata(iUnitIn)
    ...
    use ModConst, ONLY: cGravitation, cAU, mSun !aav
    use ModUser, ONLY : OmegaOrbit_dim, Rorbit_au!aav
    ...
    ! BODYDENSITY
    write(stmp,'(es12.2)')BodyNDim_I(IonFirst_) !!aav change the format of number
    ...
    ! rPlanetSi !!aav change the format of number
    write(stmp,'(es12.2)')rPlanetSi * 100. ! rplanet in cm
    write(iUnitHere,'(a,a,a)') 'AUXDATA Rp_cm="',trim(adjustl(stmp)),'"'
    
    
    ! aav, OmegaOrbit
    write(stmp,'(es12.2)') OmegaOrbit_dim ! OmegaOrbit in seconds
    write(iUnitHere,'(a,a,a)') 'AUXDATA OmegaOrb_sec="',trim(adjustl(stmp)),'"'
    
    ! aav, Rorbit_au
    write(stmp,'(f14.6)') Rorbit_au ! Rorbit_au
    write(iUnitHere,'(a,a,a)') 'AUXDATA Rorbit_au="',trim(adjustl(stmp)),'"'
    
    !aav, Mstar
    write(stmp,'(f14.6)')  OmegaOrbit_dim**2 * (Rorbit_au*cAU)**3 / (cGravitation* mSun) ! Mstar
