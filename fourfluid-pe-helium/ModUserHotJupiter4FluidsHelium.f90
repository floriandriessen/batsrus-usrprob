!==============================================================================
!    Four-fluid (M)HD model for atmospheric escape of extrasolar gas giants.
!==============================================================================
! The planetary atmosphere consists of the following fluids:
!   > singly ionised hydrogen and helium
...
!
! Fluids interact by means of collisions and mass-loading within the 5-moment
! approximation. Included are:
!   > Ion-neutral resonant elastic collisions (resonant charge-exchange)
!   > Ion-neutral non-resonant elastic collisions (charge-exchange)
!   > Ion-ion Coulomb collisions
!   > Electron-ion and ion-electron Coulomb collisions
!   > Neutral-neutral hard-sphere collisions
!   > Electron-neutral and neutral-electron hard-sphere collisions
!   > Photo-ionisation, e-impact ionisation, and recombination
!
! Collisions between particles of the same microstate do not contribute in
! the 5-moment approximation.
!
...
!
! HISTORY
!   Aug 2023 : init from two-fluid code
!==============================================================================
module ModUser
  ...
  character(len=*), parameter :: NameUserModule = &
       'Four-fluid + Pe (M)HD simulation for extrasolar gas giants'

  ! Convenient for some arrays (ordering species needs to be as in ModEquation)
  integer, parameter :: iHp_ = 1, iHep_ = 2, iH1_ = 3, iHe4_ = 4
  ...
  real :: TauCoefH1, TauCoefHe4
  real, parameter :: IonisationEnergyH1Ev = 13.6, IonisationEnergyHe4Ev = 24.6

  ! Coefficients for simplifying numerical factors in rate equations
  real :: PhotoIonisationCoefH1, PhotoIonisationCoefHe4
  real :: ElectronImpactCoefH1, ElectronImpactCoefHe4, RecombinationCoef
  real :: HeatPhotoCoefH1, HeatPhotoCoefHe4, CoolLyalphaCoef
  real :: CxCoef, CxCoefHe, CxCoefH1Hep, CxCoefHpHe4

  ! For photo-ionisation
  real :: PhotoCrossSectionH1, PhotoCrossSectionHe4
  ...
  real, allocatable :: &
       ...
       PlotExp_mTauHe_CB(:,:,:,:), PlotHeatHe_CB(:,:,:,:),    &
       PlotRatePhotoIonHe_CB(:,:,:,:), PlotRateCollIonHe_CB(:,:,:,:)
  ...
  real, public :: RhoRadGrid_C(nRadGridX, nRadGridY, nRadGridZ, 2)=0.0
  real, public :: ExpTauRadGrid_C(nRadGridX, nRadGridY, nRadGridZ, 2)=1.0

  ! Rate coefficients (cm3 K^1.5/s) of charged collisions for a CoulombLog = 16
  ! plasma (Schunk & Nagy 2009, table 4.3 is for CoulombLog = 15 plasma)
  real, parameter :: IonIonCoefDim_II(nIonFluid,nIonFluid) = reshape( &
       ! H+    He+
       [ 0.0,  1.22,   & ! H+
         0.30, 0.0  ], & ! He+
       shape=[2,2], order=[2,1] )
  real :: IonIonCoef_II(nIonFluid,nIonFluid), ElectronIonCoef
  real :: InvMassIon_I(nIonFluid), InvMassIonIon_II(nIonFluid,nIonFluid)

  ! Convenient for multi-neutrals
  integer, parameter :: nNeuFluid = nFluid - NeutralFirst_ + 1
  integer, parameter :: &
       iRhoNeu_I(nNeuFluid)   = iRho_I(NeutralFirst_:nFluid),   &
       iRhoUxNeu_I(nNeuFluid) = iRhoUx_I(NeutralFirst_:nFluid), &
       iRhoUyNeu_I(nNeuFluid) = iRhoUy_I(NeutralFirst_:nFluid), &
       iRhoUzNeu_I(nNeuFluid) = iRhoUz_I(NeutralFirst_:nFluid), &
       iPNeu_I(nNeuFluid)     = iP_I(NeutralFirst_:nFluid)

  ! Hard-sphere collision cross-section (m2) from van der Waals radius:
  ! Bondi (1964), table 1
  real, parameter :: NeuNeuCoefDim_II(nNeuFluid,nNeuFluid) = reshape( &
       ! H1       He4
       [ 0.0,     260e-12,   & ! H1
         260e-12, 0.0     ], & ! He4
       shape=[2,2], order=[2,1] )
  real, parameter :: ElectronNeuCoefDim_I(nNeuFluid) = [ 120e-12, 140e-12 ]
  real :: NeuNeuCoef_II(nNeuFluid,nNeuFluid), ElectronNeuCoef_I(nNeuFluid)
  real :: MassNeu_I(nNeuFluid), InvMassNeu_I(nNeuFluid)
  real :: InvMassNeuNeu_II(nNeuFluid,nNeuFluid)

  ! Rate coefficient (cm3/s) and Bprime factor for resonant charge exchange
  ! (Schunk & Nagy 2009, table 4.5). Collision rate is written in the form
  ! Nu = RateCoef * T_in^1/2 * n_n * (1 - Bprime * log10(T_in))^2
  real, parameter :: CxCoefDim_II(nIonFluid,nNeuFluid) = reshape( &
       ! H1        He4
       [ 2.65e-10, 0.0,        & ! H+
         0.0,      8.73e-11 ], & ! He+
       shape=[2,2], order=[2,1] )
  real, parameter :: Bprime_II(nIonFluid,nNeuFluid) = reshape( &
       ! H1     He4
       [ 0.083, 0.0,     & ! H+
         0.0,   0.093 ], & ! He+
       shape=[2,2], order=[2,1] )
  real :: CxCoef_II(nIonFluid,nNeuFluid)
  real :: InvMassIonNeu_II(nIonFluid,nNeuFluid)
...
!==============================================================================
  subroutine user_io_units
    ...
    ! NOTE: UnitEnergy = UnitEnergydens because No2Io(UnitX) = 1
...
!==============================================================================
  subroutine user_normalization
    ...
    No2Si_V(UnitU_)   = sqrt(Gamma * cBoltzmann * BodyTDim_I(iHp_)/cProtonMass)
...
!==============================================================================
  subroutine user_init_session
    ...
    ! Photo-ionisation cross-section (cm2)
    PhotoCrossSectionH1  = compute_cross_section('h1', EphotEv)
    PhotoCrossSectionHe4 = compute_cross_section('he4', EphotEv)
    ...
    PhotoIonisationCoefH1 = PhotoCrossSectionH1 * FxuvCgs / EphotCgs &
         / Io2No_V(UnitT_)
    PhotoIonisationCoefHe4 = PhotoCrossSectionHe4 * FxuvCgs / EphotCgs &
         / Io2No_V(UnitT_)

    ! Heating coefficient (erg/cm3/s)
    ! Fraction photon energy to be deposited as heat
    EpsilonNu = 1.0 - (IonisationEnergyH1Ev / EphotEv)
    HeatPhotoCoefH1 = EpsilonNu * PhotoCrossSectionH1 * FxuvCgs &
         * No2Io_V(UnitN_) * Io2No_V(UnitEnergyDens_) / Io2No_V(UnitT_)

    EpsilonNu = 1.0 - (IonisationEnergyHe4Ev / EphotEv)
    HeatPhotoCoefHe4 = EpsilonNu * PhotoCrossSectionHe4 * FxuvCgs &
         * No2Io_V(UnitN_) * Io2No_V(UnitEnergyDens_) / Io2No_V(UnitT_)

    ! Constant optical depth (factor 100 is to go from m -> cm)
    TauCoefH1  = PhotoCrossSectionH1 * No2Io_V(UnitN_) / MassFluid_I(iH1_) &
         * 100.0 * No2Si_V(UnitX_)
    TauCoefHe4 = PhotoCrossSectionHe4 * No2Io_V(UnitN_) / MassFluid_I(iHe4_) &
         * 100.0 * No2Si_V(UnitX_)
    ...
    ElectronImpactCoefH1 = 0.291e-7 * No2Io_V(UnitN_) / Io2No_V(UnitT_)
    ElectronImpactCoefHe4 = 0.175e-7 * No2Io_V(UnitN_) / Io2No_V(UnitT_)
    ...
    CxCoef_II = CxCoefDim_II * sqrt(No2Si_V(UnitTemperature_)) &
         * No2Io_V(UnitN_) / Io2No_V(UnitT_)

    ! Non-resonant charge-exchange rate (1/s)
    CxCoefH1Hep = 1.25e-15 * No2Io_V(UnitN_) / Io2No_V(UnitT_)
    CxCoefHpHe4 = 1.75e-11 * No2Io_V(UnitN_) / Io2No_V(UnitT_)

    ! Ion-ion collision rate coefficients (1/s)
    IonIonCoef_II = IonIonCoefDim_II * No2Io_V(UnitN_) &
         / No2Io_V(UnitTemperature_)**1.5 / Io2No_V(UnitT_)

    ! Electron-ion collision coefficient to simplify collision frequency:
    ! nuElectronIon = ElectronIonCoef * Nion * Zion^2 / Te^1.5
    ElectronIonCoef = CoulombLog * (cElectronCharge**2/cEps)**2 &
         / ( 3.0*sqrt(cElectronMass) * (2.0*cPi*cBoltzmann)**1.5 )
    ElectronIonCoef = ElectronIonCoef * No2Si_V(UnitN_) &
         / No2Si_V(UnitTemperature_)**1.5 / Si2No_V(UnitT_)

    ! Neutral-neutral collision cross sections
    NeuNeuCoef_II = cPi * NeuNeuCoefDim_II**2 * Si2No_V(UnitX_)**2

    ! Electron-neutral collision coefficient
    ElectronNeuCoef_I = cPi * ElectronNeuCoefDim_I**2 * Si2No_V(UnitX_)**2

    ! Useful for ion and neutral collision terms
    InvMassIon_I = 1.0/MassIon_I
    MassNeu_I    = MassFluid_I(NeutralFirst_:nFluid)
    InvMassNeu_I = 1.0/MassNeu_I

    do iIon = 1,nIonFluid
      InvMassIonIon_II(iIon,:) = 1.0/(MassIon_I(iIon) + MassIon_I)
      InvMassIonNeu_II(iIon,:) = 1.0/(MassIon_I(iIon) + MassNeu_I)
    enddo

    do iNeu = 1,nNeuFluid
      InvMassNeuNeu_II(iNeu,:) = 1.0/(MassNeu_I(iNeu) + MassNeu_I)
    enddo
...
!==============================================================================
! Compute the cross section of a given atom/ion using eq. 1 and table 1 of
! Verner+ (1996), ApJ 465. Cross section is expressed in cgs units.
!==============================================================================
  pure elemental function compute_cross_section(Specie, EphotEv) result(Sigma)

    character(*), intent(in) :: Specie
    real,         intent(in) :: EphotEv

    ! Local variables following the same convention as in the paper
    real :: x, E0, y0, y1, y, yw, P, ya, Sigma0, Sigma
    ...
    Sigma = Sigma0 * 1e-18 * ((x - 1.0)**2 + yw**2) * y**(0.5*P - 5.5) &
         / ( 1.0 + sqrt(y/ya) )**P

  end function compute_cross_section
...
!==============================================================================
  subroutine user_set_face_boundary(VarsGhostFace_V)
    ...
    VarsGhostFace_V(Pe_)    = BodyP_I(iHp_) + BodyP_I(iHep_)
...
! PARAM.in the #OUTERBOUNDARY is 'userfloat'.
!==============================================================================
    ...
    subroutine init_uniform_3d_grid_rho
      ...
      do k = 1,nRadGridZ; do j = 1,nRadGridY; do i = 1,nRadGridX
        ...
        if (rRadGrid <= rBody) then
          RhoRadGrid_C(i,j,k,:) = BodyRho_I(iH1_:iHe4_)
          CYCLE
        end if
        ...
        RhoRadGrid_C(i,j,k,:) = BodyRho_I(iH1_:iHe4_) * Vrad0Pw/Vrad &
             * (rBody/rRadGrid)**2
      enddo; enddo; enddo
      ...
      RhoRadGrid_C(1:nRadGridX,1:nRadGridY,1:nRadGridZ,1:2) = 0.0
...
!==============================================================================
  subroutine user_init_point_implicit
    ...
    iVarPointImpl_I(5*nFluid+1) = Pe_
    do iFluid = 1, nFluid
...
!==============================================================================
  subroutine user_calc_sources(iBlock)
    ...
    use ModConst,         ONLY: cPi, cElectronMass, cKToEV
    use BATL_lib,         ONLY: nI, nJ, nK
    ...
    real :: PhotoIonisationRateH1, PhotoIonisationRateHe4
    real :: ElectronImpactRateH1, ElectronImpactRateHe4, RecombinationRate
    real :: TotIonisationRateH1, TotIonisationRateHe4
    real :: HeatPhotoIonH1, HeatPhotoIonHe4, CoolLyalpha
    real :: CollisionCoef, ExpMinTauH1, ExpMinTauHe4, Ufrac
    real :: GravityStar_D(3), ForceCentrifugal_D(3)
    real :: ElasticCollisionCrossSection, SfacSq
    real :: NuCx, NuCxHe, RateH1Hep, RateHpHe4
    real :: NuElectronHp, NuElectronHep, NuElectronH1, NuElectronHe4

    ! NEW
    real :: NumDensIon_I(nIonFluid), NumDensHep, NumDensHe4, SpHep, SpHe4, Tred
    real :: uHep_D(3), uHe4_D(3), TH1, THp, THe4, Thep
    real :: yAbsorb, UxIon_I(nIonFluid), UyIon_I(nIonFluid), UzIon_I(nIonFluid)

    real :: TempIonIon_II(nIonFluid,nIonFluid), TempIon_I(nIonFluid)
    real :: NuIonIon_II(nIonFluid,nIonFluid), UfricIon_I(nIonFluid)
    real :: UfricNeu_I(nNeuFluid), TempNeu_I(nIonFluid)
    integer :: iIon, iRhoIon, iRhoUxIon, iRhoUyIon, iRhoUzIon, iPIon
    real :: RhoElectron, NuElectronIon_I(nIonFluid)
    real :: UxNeu_I(nNeuFluid), UyNeu_I(nNeuFluid), UzNeu_I(nNeuFluid)
    real :: NumDensNeu_I(nNeuFluid), NuElectronNeu_I(nNeuFluid)
    integer :: iNeu, iRhoNeu, iRhoUxNeu, iRhoUyNeu, iRhoUzNeu, iPNeu
    real :: TempNeuNeu_II(nNeuFluid,nNeuFluid), NuNeuNeu_II(nNeuFluid,nNeuFluid)

    real :: NuCx_II(nIonFluid,nNeuFluid), UfricCx_II(nIonFluid,nNeuFluid)
    real :: TempCx_II(nIonFluid,nNeuFluid)
    real :: Rfric_DII(3,nIonFluid,nNeuFluid), Qfric_II(nIonFluid,nNeuFluid)
    real :: Qtot_II(nIonFluid,nNeuFluid)
  ...
  contains
    ...
    subroutine user_implicit_sources
      ...
      do k = 1,nK; do j = 1,nJ; do i = 1,nI
        ...
        NumDensIon_I    = State_V(iRhoIon_I) / MassIon_I
        NumDensNeu_I    = State_V(iRhoNeu_I) / MassNeu_I
        NumDensHp       = State_V(HpRho_) / MassFluid_I(iHp_)
        NumDensHep      = State_V(HepRho_) / MassFluid_I(iHep_)
        NumDensH1       = State_V(H1Rho_) / MassFluid_I(iH1_)
        NumDensHe4      = State_V(He4Rho_) / MassFluid_I(iHe4_)
        NumDensElectron = sum(ChargeIon_I * NumDensIon_I)
        RhoElectron     = mElectron * NumDensElectron

        uHp_D  = State_V(HpRhoUx_:HpRhoUz_) / State_V(HpRho_)
        uHep_D = State_V(HepRhoUx_:HepRhoUz_) / State_V(HepRho_)
        uH1_D  = State_V(H1RhoUx_:H1RhoUz_) / State_V(H1Rho_)
        uHe4_D = State_V(He4RhoUx_:He4RhoUz_) / State_V(He4Rho_)

        UxIon_I = State_V(iRhoUxIon_I) / State_V(iRhoIon_I)
        UyIon_I = State_V(iRhoUyIon_I) / State_V(iRhoIon_I)
        UzIon_I = State_V(iRhoUzIon_I) / State_V(iRhoIon_I)
        UxNeu_I = State_V(iRhoUxNeu_I) / State_V(iRhoNeu_I)
        UyNeu_I = State_V(iRhoUyNeu_I) / State_V(iRhoNeu_I)
        UzNeu_I = State_V(iRhoUzNeu_I) / State_V(iRhoNeu_I)

        ! Electron velocity = charge-averaged ion velocity
        uE_D = [ sum(ChargeIon_I * NumDensIon_I * UxIon_I),  &
                 sum(ChargeIon_I * NumDensIon_I * UyIon_I),  &
                 sum(ChargeIon_I * NumDensIon_I * UzIon_I) ] &
                / NumDensElectron

        ! Ion and neutral (code units), and electron temperature (Kelvin)
        THp       = State_V(HpP_) / NumDensHp
        TH1       = State_V(H1P_) / NumDensH1
        THep      = State_V(HepP_) / NumDensHep
        THe4      = State_V(He4P_) / NumDensHe4
        Telectron = State_V(Pe_) / NumDensElectron * No2Si_V(UnitTemperature_)
        TempIon_I = State_V(iPIon_I) / NumDensIon_I
        TempNeu_I = State_V(iPNeu_I) / NumDensNeu_I
        ...
        if (r_BLK(i,j,k,iBlock) <= rBody) then
          ...
          ExpMinTauH1 = &
               interpolate_scalar(ExpTauRadGrid_C(:,:,:,1), 3, [ 1, 1, 1 ],   &
                                  [ nRadGridX, nRadGridY, nRadGridZ ],        &
                                  [ x, y, z ],                                &
                                  RadGridX_C(:), RadGridY_C(:), RadGridZ_C(:),&
                                  DoExtrapolate)
          ExpMinTauHe4 = &
               interpolate_scalar(ExpTauRadGrid_C(:,:,:,2), 3, [ 1, 1, 1 ],   &
                                  [ nRadGridX, nRadGridY, nRadGridZ ],        &
                                  [ x, y, z ],                                &
                                  RadGridX_C(:), RadGridY_C(:), RadGridZ_C(:),&
                                  DoExtrapolate)
        ...
        !> Hydrogen
        !>
        ! Fraction of Hydrogen absorbing the incoming stellar photon
        ! Osterbrock & Ferland (2006), eq. 2.21
        yAbsorb = PhotoIonisationCoefH1 * NumDensH1 &
             / ( PhotoIonisationCoefH1 * NumDensH1  &
             +   PhotoIonisationCoefHe4 * NumDensHe4 )

        ! 1. Photo-ionisation rate by radiation: H1 + hv -> H+ + e- + Eheat
        PhotoIonisationRateH1 = yAbsorb * PhotoIonisationCoefH1 * ExpMinTauH1

        ! 1.1 Photo-ionising heat carried away by the electron
        HeatPhotoIonH1 = yAbsorb * HeatPhotoCoefH1 * NumDensH1 * ExpMinTauH1

        ! 2. Electron-impact ionisation: H1 + e- -> H+ + 2e-
        ! Fit from Voronov (1997), eq. 1 + table 1
        Ufrac = IonisationEnergyH1Ev / (cKToEV * Telectron)
        ElectronImpactRateH1 = ElectronImpactCoefH1 * NumDensElectron &
             * Ufrac**0.39 / (0.232 + Ufrac) * exp(-Ufrac)

        ! 3. Radiative (ion-electron) recombination: H+ + e- -> H1 + hv
        ! Fit from Storey & Hummer (1995), table 1
        RecombinationRate = RecombinationCoef * (1.0e4/Telectron)**0.9
        ...
        TotIonisationRateH1 = PhotoIonisationRateH1 + ElectronImpactRateH1

        ! Rate per volume at which H+ mass is produced from H1 mass
        SrhoSpecies = TotIonisationRateH1 * State_V(H1Rho_) &
             - RecombinationRate * State_V(HpRho_) * NumDensElectron
        ...
        Smom_D = TotIonisationRateH1 * State_V(H1Rho_) * uH1_D &
             - RecombinationRate * State_V(HpRho_) * NumDensElectron * uHp_D
        ...
        SpHp = TotIonisationRateH1 * ( InvGammaMinus1 * State_V(H1P_) &
        ...
        SpH1 = RecombinationRate * NumDensElectron * ( InvGammaMinus1         &
             * State_V(HpP_) + 0.5*State_V(HpRho_) * sum((uH1_D - uHp_D)**2) )&
             - InvGammaMinus1 * TotIonisationRateH1 * State_V(H1P_)
        ...
        SpE = -InvGammaMinus1 * RecombinationRate * State_V(Pe_) * NumDensHp &
             + 0.5*mElectron * NumDensH1 * TotIonisationRateH1               &
             * sum((uH1_D - uE_D)**2) + HeatPhotoIonH1 - CoolLyalpha
        ...
        !> Helium
        !>
        ! 1. Photo-ionisation rate by radiation: He + hv -> He+ + e- + Eheat
        PhotoIonisationRateHe4 = (1.0 - yAbsorb) * PhotoIonisationCoefHe4 &
             * ExpMinTauHe4

        ! 1.1 Photo-ionising heat carried away by the electron
        HeatPhotoIonHe4 = (1.0 - yAbsorb) * HeatPhotoCoefHe4 * NumDensHe4 &
             * ExpMinTauHe4

        ! 2. Electron-impact ionisation: He + e- -> He+ + 2e-
        ! Fit from Voronov (1997), eq. 1 + table 1
        Ufrac = IonisationEnergyHe4Ev / (cKToEV * Telectron)
        ElectronImpactRateHe4 = ElectronImpactCoefHe4 * NumDensElectron &
             * Ufrac**0.35 / (0.180 + Ufrac) * exp(-Ufrac)

        ! 3. Radiative (ion-electron) recombination: He+ + e- -> He4 + hv
        ! Fit from Storey & Hummer (1995), table 1
        RecombinationRate = RecombinationCoef * (1.0e4/Telectron)**0.9

        ! 5. Update hydro equations
        TotIonisationRateHe4 = PhotoIonisationRateHe4 + ElectronImpactRateHe4

        ! Rate per volume at which He+ mass is produced from He4 mass
        SrhoSpecies = TotIonisationRateHe4 * State_V(He4Rho_) &
             - RecombinationRate * State_V(HepRho_) * NumDensElectron

        Source_VC(HepRho_,i,j,k) = Source_VC(HepRho_,i,j,k) + SrhoSpecies
        Source_VC(He4Rho_,i,j,k) = Source_VC(He4Rho_,i,j,k) - SrhoSpecies

        ! Rate per volume of momentum transfer to the He+ fluid
        Smom_D = TotIonisationRateHe4 * State_V(He4Rho_) * uHe4_D &
             - RecombinationRate * State_V(HepRho_) * NumDensElectron * uHep_D

        Source_VC(HepRhoUx_:HepRhoUz_,i,j,k) = &
             Source_VC(HepRhoUx_:HepRhoUz_,i,j,k) + Smom_D
        Source_VC(He4RhoUx_:He4RhoUz_,i,j,k) = &
             Source_VC(He4RhoUx_:He4RhoUz_,i,j,k) - Smom_D

        ! Rate per volume at which THERMAL energy is added to the He+ fluid
        SpHep = TotIonisationRateHe4 * ( InvGammaMinus1 * State_V(He4P_) &
             + 0.5*State_V(He4Rho_) * sum((uHe4_D - uHep_D)**2) )        &
             - InvGammaMinus1 * RecombinationRate * State_V(HepP_)       &
             * NumDensElectron

        ! Rate per volume at which THERMAL energy is added to the He4 fluid
        SpHe4 = RecombinationRate * NumDensElectron               &
             * ( InvGammaMinus1 * State_V(HepP_)                  &
             + 0.5*State_V(HepRho_) * sum((uHe4_D - uHep_D)**2) ) &
             - InvGammaMinus1 * TotIonisationRateHe4 * State_V(He4P_)

        Source_VC(HepP_,i,j,k) = Source_VC(HepP_,i,j,k) + GammaMinus1 * SpHep
        Source_VC(He4P_,i,j,k) = Source_VC(He4P_,i,j,k) + GammaMinus1 * SpHe4

        ! Rate per volume at which THERMAL energy is added to the electrons
        SpE = -InvGammaMinus1 * RecombinationRate * State_V(Pe_) * NumDensHep &
             + 0.5*mElectron * NumDensHe4 * TotIonisationRateHe4              &
             * sum((uHe4_D - uE_D)**2) + HeatPhotoIonHe4

        Source_VC(Pe_,i,j,k) = Source_VC(Pe_,i,j,k) + GammaMinus1 * SpE

        ! Store for printing variables
        PlotRateRec_CB(i,j,k,iBlock)        = 2*RecombinationRate
        PlotRateCollIon_CB(i,j,k,iBlock)    = ElectronImpactRateH1
        PlotRateCollIonHe_CB(i,j,k,iBlock)  = ElectronImpactRateHe4
        PlotRatePhotoIon_CB(i,j,k,iBlock)   = PhotoIonisationRateH1
        PlotRatePhotoIonHe_CB(i,j,k,iBlock) = PhotoIonisationRateHe4
        PlotHeat_CB(i,j,k,iBlock)           = HeatPhotoIonH1
        PlotHeatHe_CB(i,j,k,iBlock)         = HeatPhotoIonHe4
        PlotCoolLya_CB(i,j,k,iBlock)        = CoolLyalpha
        PlotExp_mTau_CB(i,j,k,iBlock)       = ExpMinTauH1
        PlotExp_mTauHe_CB(i,j,k,iBlock)     = ExpMinTauHe4

        !*****************************************
        ! Ion-neutral resonant elastic collisions
        !*****************************************
        ! Note below only works at the moment for resonant charge exchange
        ! between a single ionised specie and its neutral parent such that
        ! iIonFluid = nNeufluid (e.g. A+ and A, but not A++ and A)
        do iIon = 1,nIonFluid
          iRhoIon   = iRhoIon_I(iIon)
          iRhoUxIon = iRhoUxIon_I(iIon)
          iRhoUyIon = iRhoUyIon_I(iIon)
          iRhoUzIon = iRhoUzIon_I(iIon)
          iPIon     = iPIon_I(iIon)

          TempCx_II(iIon,:) = 0.5*(TempIon_I(iIon) + TempNeu_I)

          ! Ion-neutral resonant charge exchange rate
          ! Note: log10 factor is normalised to 1 Kelvin for unit consistency
          NuCx_II(iIon,:) = CxCoef_II(iIon,:) * NumDensNeu_I &
               * sqrt(TempCx_II(iIon,:)) * ( 1.0 - Bprime_II(iIon,:) &
               * log10(TempCx_II(iIon,:)*No2Si_V(UnitTemperature_)) )**2

          ! Rate per volume of momentum transfer to the iIon fluid
          Rfric_DII(x_,iIon,:) = State_V(iRhoIon) * NuCx_II(iIon,:) &
               * (UxNeu_I - UxIon_I(iIon))
          Rfric_DII(y_,iIon,:) = State_V(iRhoIon) * NuCx_II(iIon,:) &
               * (UyNeu_I - UyIon_I(iIon))
          Rfric_DII(z_,iIon,:) = State_V(iRhoIon) * NuCx_II(iIon,:) &
               * (UzNeu_I - UzIon_I(iIon))

          Source_VC(iRhoUxIon,i,j,k) = Source_VC(iRhoUxIon,i,j,k) &
               + sum(Rfric_DII(x_,iIon,:))
          Source_VC(iRhoUyIon,i,j,k) = Source_VC(iRhoUyIon,i,j,k) &
               + sum(Rfric_DII(y_,iIon,:))
          Source_VC(iRhoUzIon,i,j,k) = Source_VC(iRhoUzIon,i,j,k) &
               + sum(Rfric_DII(z_,iIon,:))

          ! Rate per volume at which thermal energy is added to iIon fluid
          UfricCx_II(iIon,:) = (UxNeu_I - UxIon_I(iIon))**2 &
               + (UyNeu_I - UyIon_I(iIon))**2 + (UzNeu_I - UzIon_I(iIon))**2

          Qfric_II(iIon,:) = State_V(iRhoIon) * NuCx_II(iIon,:)               &
               * InvMassIonNeu_II(iIon,:) * (3.0*(TempNeu_I - TempIon_I(iIon))&
               + MassNeu_I * UfricCx_II(iIon,:))

          Source_VC(iPIon,i,j,k) = Source_VC(iPIon,i,j,k) + GammaMinus1 &
               * sum(Qfric_II(iIon,:))

          ! Needed for neutrals below
          Qtot_II(iIon,:) = Rfric_DII(x_,iIon,:) * (UxNeu_I - UxIon_I(iIon)) &
               + Rfric_DII(y_,iIon,:) * (UyNeu_I - UyIon_I(iIon))            &
               + Rfric_DII(z_,iIon,:) * (UzNeu_I - UzIon_I(iIon))
        enddo

        ! Rate per volume of momentum transfer to the neutral fluids
        Source_VC(iRhoUxNeu_I,i,j,k) = Source_VC(iRhoUxNeu_I,i,j,k) &
             - sum(Rfric_DII(x_,:,:), dim=2)
        Source_VC(iRhoUyNeu_I,i,j,k) = Source_VC(iRhoUyNeu_I,i,j,k) &
             - sum(Rfric_DII(y_,:,:), dim=2)
        Source_VC(iRhoUzNeu_I,i,j,k) = Source_VC(iRhoUzNeu_I,i,j,k) &
             - sum(Rfric_DII(z_,:,:), dim=2)

        ! Rate per volume at which thermal energy is added to neutral fluids:
        ! Q_in + Q_ni = vec(R)_in * [vec(u)_n - vec(u)_i]
        Source_VC(iPNeu_I,i,j,k) = Source_VC(iPNeu_I,i,j,k) + GammaMinus1 &
             * ( sum(Qtot_II(:,:), dim=1) - sum(Qfric_II(:,:), dim=1) )

        !******************************************
        ! Ion-neutral non-resonant charge-exchange
        !******************************************
        !>
        !> H1 + He+ -> H+ + He4
        !>
        Tred = (MassFluid_I(iH1_)*THep + MassFluid_I(iHep_)*TH1) &
             / (MassFluid_I(iH1_) + MassFluid_I(iHep_))          &
             * No2Si_V(UnitTemperature_)
        RateH1Hep = CxCoefH1Hep * (Tred/300.0)**0.25

        ! Rate per volume at which H+ mass is produced
        SrhoSpecies = State_V(H1Rho_) * NumDensHep * RateH1Hep &
             - State_V(HpRho_) * NumDensHe4 * RateHpHe4

        Source_VC(HpRho_,i,j,k) = Source_VC(HpRho_,i,j,k) + SrhoSpecies
        Source_VC(H1Rho_,i,j,k) = Source_VC(H1Rho_,i,j,k) - SrhoSpecies

        ! Rate per volume of momentum transfer to the H+ fluid
        Smom_D = State_V(H1Rho_) * NumDensHep * RateH1Hep * uH1_D &
             - State_V(HpRho_) * NumDensHe4 * RateHpHe4 * uHp_D
        ...
        ! Rate per volume at which THERMAL energy is added to the H+ fluid
        SpHp = -State_V(HpP_) * NumDensHp * NumDensHe4 * RateHpHe4 &
             + 0.5*State_V(H1Rho_) * NumDensHep * RateH1Hep        &
             * sum((uH1_D - uHp_D)**2)

        ! Rate per volume at which THERMAL energy is added to the H1 fluid
        SpH1 = -State_V(H1P_) * NumDensH1 * NumDensHep * RateH1Hep &
             + 0.5*State_V(HpRho_) * NumDensHe4 * RateHpHe4        &
             * sum((uH1_D - uHp_D)**2)
        ...
        !> H+ + He4 -> H1 + He+
        !>
        Tred = (MassFluid_I(iHp_)*THe4 + MassFluid_I(iHe4_)*THp) &
             / (MassFluid_I(iHp_) + MassFluid_I(iHe4_))          &
             * No2Si_V(UnitTemperature_)
        RateHpHe4 = CxCoefHpHe4 * (300.0/Tred)**0.75 * exp(-127500.0/Tred)

        ! Rate per volume at which He+ mass is produced
        SrhoSpecies = State_V(He4Rho_) * NumDensHp * RateHpHe4 &
             - State_V(HepRho_) * NumDensH1 * RateH1Hep

        Source_VC(HepRho_,i,j,k) = Source_VC(HepRho_,i,j,k) + SrhoSpecies
        Source_VC(He4Rho_,i,j,k) = Source_VC(He4Rho_,i,j,k) - SrhoSpecies

        ! Rate per volume of momentum transfer to the He+ fluid
        Smom_D = State_V(He4Rho_) * NumDensHp * RateHpHe4 * uHe4_D &
             - State_V(HepRho_) * NumDensH1 * RateH1Hep * uHep_D
        ...
        ! Rate per volume at which THERMAL energy is added to the He+ fluid
        SpHep = -State_V(HepP_) * NumDensHep * NumDensH1 * RateH1Hep &
             + 0.5*State_V(He4Rho_) * NumDensHp * RateHpHe4          &
             * sum((uHe4_D - uHep_D)**2)

        ! Rate per volume at which THERMAL energy is added to the He4 fluid
        SpHe4 = -State_V(He4P_) * NumDensHe4 * NumDensHp * RateHpHe4 &
             + 0.5*State_V(HepRho_) * NumDensH1 * RateH1Hep          &
             * sum((uHe4_D - uHep_D)**2)
        ...
        ! Ion-ion collisions
        !********************
        ! Note that collisions of same microstates yield no contribution below
        do iIon = 1,nIonFluid
          iRhoIon   = iRhoIon_I(iIon)
          iRhoUxIon = iRhoUxIon_I(iIon)
          iRhoUyIon = iRhoUyIon_I(iIon)
          ...
          TempIonIon_II(iIon,:) = InvMassIonIon_II(iIon,:) &
               * ( MassIon_I(iIon)*TempIon_I + MassIon_I*TempIon_I(iIon) )

          ! Ion-ion collision rate
          NuIonIon_II(iIon,:) = IonIonCoef_II(iIon,:) * NumDensIon_I &
               / TempIonIon_II(iIon,:)**1.5

          ! Rate per volume of momentum transfer to the iIon fluid
          Source_VC(iRhoUxIon,i,j,k) = Source_VC(iRhoUxIon,i,j,k) &
               + sum( State_V(iRhoIon) * NuIonIon_II(iIon,:)      &
               *      (UxIon_I - UxIon_I(iIon)) )

          Source_VC(iRhoUyIon,i,j,k) = Source_VC(iRhoUyIon,i,j,k) &
               + sum( State_V(iRhoIon) * NuIonIon_II(iIon,:)      &
               *      (UyIon_I - UyIon_I(iIon)) )

          Source_VC(iRhoUzIon,i,j,k) = Source_VC(iRhoUzIon,i,j,k) &
               + sum( State_V(iRhoIon) * NuIonIon_II(iIon,:)      &
               *      (UzIon_I - UzIon_I(iIon)) )

          ! Rate per volume at which thermal energy is added to iIon fluid
          UfricIon_I = (UxIon_I - UxIon_I(iIon))**2 &
               + (UyIon_I - UyIon_I(iIon))**2 + (UzIon_I - UzIon_I(iIon))**2

          Source_VC(iPIon,i,j,k) = Source_VC(iPIon,i,j,k) + GammaMinus1       &
               * sum( State_V(iRhoIon) * NuIonIon_II(iIon,:)                  &
               * InvMassIonIon_II(iIon,:) * (3.0*(TempIon_I - TempIon_I(iIon))&
               + MassIon_I * UfricIon_I) )
        enddo
        ...
        ! Note that collisions of same microstates yield no contribution below
        do iNeu = 1,nNeuFluid
          iRhoNeu   = iRhoNeu_I(iNeu)
          iRhoUxNeu = iRhoUxNeu_I(iNeu)
          iRhoUyNeu = iRhoUyNeu_I(iNeu)
          iRhoUzNeu = iRhoUzNeu_I(iNeu)
          iPNeu     = iPNeu_I(iNeu)

          ! This is actually T_red / m_red for easier notation below
          TempNeuNeu_II(iNeu,:) = TempNeu_I / MassNeu_I &
               + TempNeu_I(iNeu) / MassNeu_I(iNeu)

          UfricNeu_I = (UxNeu_I - UxNeu_I(iNeu))**2 &
               + (UyNeu_I - UyNeu_I(iNeu))**2 + (UzNeu_I - UzNeu_I(iNeu))**2

          ! Neutral-neutral collision rate
          NuNeuNeu_II(iNeu,:) = 8.0/3.0 * NeuNeuCoef_II(iNeu,:) &
               * State_V(iRhoNeu_I) * InvMassNeuNeu_II(iNeu,:)  &
               * sqrt(2.0/cPi * TempNeuNeu_II(iNeu,:))          &
               * sqrt(1.0+9.0*cPi/64.0 * 0.5*UfricNeu_I/TempNeuNeu_II(iNeu,:))

          ! Rate per volume of momentum transfer to the iNeu fluid
          Source_VC(iRhoUxNeu,i,j,k) = Source_VC(iRhoUxNeu,i,j,k) &
               + sum( State_V(iRhoNeu) * NuNeuNeu_II(iNeu,:)      &
               *      (UxNeu_I - UxNeu_I(iNeu)) )

          Source_VC(iRhoUyNeu,i,j,k) = Source_VC(iRhoUyNeu,i,j,k) &
               + sum( State_V(iRhoNeu) * NuNeuNeu_II(iNeu,:)      &
               *      (UyNeu_I - UyNeu_I(iNeu)) )

          Source_VC(iRhoUzNeu,i,j,k) = Source_VC(iRhoUzNeu,i,j,k) &
               + sum( State_V(iRhoNeu) * NuNeuNeu_II(iNeu,:)      &
               *      (UzNeu_I - UzNeu_I(iNeu)) )

          ! Rate per volume at which thermal energy is added to the iNeu fluid
          Source_VC(iPNeu,i,j,k) = Source_VC(iPNeu,i,j,k) + GammaMinus1       &
               * sum( State_V(iRhoNeu) * NuNeuNeu_II(iNeu,:)                  &
               * InvMassNeuNeu_II(iNeu,:) * (3.0*(TempNeu_I - TempNeu_I(iNeu))&
               + MassNeu_I * UfricNeu_I) )
        enddo

        !*************************
        ! Electron-ion collisions
        !*************************
        ! Note for ion terms rho_i * nu_ie = rho_e * nu_ei is used and also
        ! in the prefactor of thermal terms m_e << m_i is taken
        Telectron = Telectron * Si2No_V(UnitTemperature_)

        ! Electron-ion collision rate
        NuElectronIon_I = ElectronIonCoef * NumDensIon_I / Telectron**1.5

        ! Rate per volume of momentum transfer to ion fluids
        Source_VC(iRhoUxIon_I,i,j,k) = Source_VC(iRhoUxIon_I,i,j,k) &
             + RhoElectron * NuElectronIon_I * (uE_D(x_) - UxIon_I)

        Source_VC(iRhoUyIon_I,i,j,k) = Source_VC(iRhoUyIon_I,i,j,k) &
             + RhoElectron * NuElectronIon_I * (uE_D(y_) - UyIon_I)

        Source_VC(iRhoUzIon_I,i,j,k) = Source_VC(iRhoUzIon_I,i,j,k) &
             + RhoElectron * NuElectronIon_I * (uE_D(z_) - UzIon_I)

        ! Rate per volume at which thermal energy is added to ion fluids
        UfricIon_I = (uE_D(x_) - UxIon_I)**2 + (uE_D(y_) - UyIon_I)**2 &
             + (uE_D(z_) - UzIon_I)**2

        Source_VC(iPIon_I,i,j,k) = Source_VC(iPIon_I,i,j,k) + GammaMinus1 &
             * ( RhoElectron * NuElectronIon_I * InvMassIon_I             &
             *   (3.0*(Telectron - TempIon_I) + mElectron * UfricIon_I) )

        ! Rate per volume at which thermal energy is added to the electrons
        Source_VC(Pe_,i,j,k)  = Source_VC(Pe_,i,j,k) + GammaMinus1 &
             * sum( RhoElectron * NuElectronIon_I * InvMassIon_I   &
             *      (3.0*(TempIon_I - Telectron) + MassIon_I * UfricIon_I) )

        !*****************************
        ! Electron-neutral collisions
        !*****************************
        ! Note for neutral terms rho_n * nu_ne = rho_e * nu_en is used and also
        ! in the prefactor of thermal terms m_e << m_n is taken
        UfricNeu_I = (uE_D(x_) - UxNeu_I)**2 + (uE_D(y_) - UyNeu_I)**2 &
             + (uE_D(z_) - UzNeu_I)**2

        NuElectronNeu_I = 8.0/3.0 * ElectronNeuCoef_I * InvMassNeu_I    &
             * State_V(iRhoNeu_I) * sqrt(2.0/cPi * Telectron/mElectron) &
             * sqrt(1.0 + 9.0*cPi/64.0 * UfricNeu_I/(2.0*Telectron/mElectron))

        ! Rate per volume of momentum transfer to neutral fluids
        Source_VC(iRhoUxNeu_I,i,j,k) = Source_VC(iRhoUxNeu_I,i,j,k) &
             + RhoElectron * NuElectronNeu_I * (uE_D(x_) - UxNeu_I)

        Source_VC(iRhoUyNeu_I,i,j,k) = Source_VC(iRhoUyNeu_I,i,j,k) &
             + RhoElectron * NuElectronNeu_I * (uE_D(y_) - UyNeu_I)

        Source_VC(iRhoUzNeu_I,i,j,k) = Source_VC(iRhoUzNeu_I,i,j,k) &
             + RhoElectron * NuElectronNeu_I * (uE_D(z_) - UzNeu_I)

        ! Rate per volume at which thermal energy is added to neutral fluids
        Source_VC(iPNeu_I,i,j,k) = Source_VC(iPNeu_I,i,j,k) + GammaMinus1 &
             * ( RhoElectron * NuElectronNeu_I * InvMassNeu_I             &
             *   (3.0*(Telectron - TempNeu_I) + mElectron * UfricNeu_I) )

        ! Rate per volume at which thermal energy is added to the electrons
        Source_VC(Pe_,i,j,k)  = Source_VC(Pe_,i,j,k) + GammaMinus1 &
             * sum( RhoElectron * NuElectronNeu_I * InvMassNeu_I   &
             *      (3.0*(TempNeu_I - Telectron) + MassNeu_I * UfricNeu_I) )
      enddo; enddo; enddo
...
!==============================================================================
  subroutine user_set_plot_var(iBlock, NameVar, IsDimensional, PlotVar_G, &
    ...
    case('tauhe')
      PlotVar_G(1:nI,1:nJ,1:nK) = PlotExp_mTauHe_CB(:,:,:,iBlock)
      !NameTecUnit = ' '
      !NameIdlUnit = ' '
      NameTecVar = 'exp(-t)_He4'
    ...
    case('heathe') ! Heating rate
      PlotVar_G(1:nI,1:nJ,1:nK) = PlotHeatHe_CB(:,:,:,iBlock)
      if (IsDimensional) then
        PlotVar_G = PlotVar_G * No2Io_V(UnitEnergyDens_) / No2Io_V(UnitT_)
      endif
      NameTecUnit = '(erg/cm^3/s)'
    ...
    case('rphotohe')
      PlotVar_G(1:nI,1:nJ,1:nK) = PlotRatePhotoIonHe_CB(:,:,:,iBlock)
      if (IsDimensional) then
        PlotVar_G = PlotVar_G / No2Io_V(UnitT_)
      endif
      NameTecUnit = '(1/s)'
      NameIdlUnit = '1/s'
      NameTecVar  = 'R_phion_He4'
    ...
    case('rcollhe')
      PlotVar_G(1:nI,1:nJ,1:nK) = PlotRateCollIonHe_CB(:,:,:,iBlock)
      if (IsDimensional) then
        PlotVar_G = PlotVar_G / No2Io_V(UnitT_)
      endif
      NameTecUnit = '(1/s)'
      NameIdlUnit = '1/s'
      NameTecVar  = 'R_collion_He4'
  ...
  subroutine user_action(NameAction)
    ...
    case('initialize module')
      ...
      if (.not.allocated(PlotExp_mTau_CB)) then
        ...
        allocate(PlotExp_mTauHe_CB(1:nI,1:nJ,1:nK,MaxBlock))
        allocate(PlotHeat_CB(1:nI,1:nJ,1:nK,MaxBlock))
        allocate(PlotHeatHe_CB(1:nI,1:nJ,1:nK,MaxBlock))
        allocate(PlotCoolLya_CB(1:nI,1:nJ,1:nK,MaxBlock))
        allocate(PlotRatePhotoIon_CB(1:nI,1:nJ,1:nK,MaxBlock))
        allocate(PlotRatePhotoIonHe_CB(1:nI,1:nJ,1:nK,MaxBlock))
        allocate(PlotRateCollIon_CB(1:nI,1:nJ,1:nK,MaxBlock))
        allocate(PlotRateCollIonHe_CB(1:nI,1:nJ,1:nK,MaxBlock))
    ...
    case('clean module')
      ...
      if (allocated(PlotExp_mTauHe_CB))     deallocate(PlotExp_mTauHe_CB)
      if (allocated(PlotHeat_CB))           deallocate(PlotHeat_CB)
      if (allocated(PlotHeatHe_CB))         deallocate(PlotHeat_CB)
      if (allocated(PlotCoolLya_CB))        deallocate(PlotCoolLya_CB)
      if (allocated(PlotRatePhotoIon_CB))   deallocate(PlotRatePhotoIon_CB)
      if (allocated(PlotRatePhotoIonHe_CB)) deallocate(PlotRatePhotoIonHe_CB)
      if (allocated(PlotRateCollIon_CB))    deallocate(PlotRateCollIon_CB)
      if (allocated(PlotRateCollIonHe_CB))  deallocate(PlotRateCollIonHe_CB)
    ...
    case('initial condition done')
      ...
      if (nProc > 1) then
        call mpi_reduce_real_array(RhoRadGrid_C(:,:,:,1), &
             size(RhoRadGrid_C(:,:,:,1)), MPI_SUM, 0, iComm, iError)
        call mpi_reduce_real_array(RhoRadGrid_C(:,:,:,2), &
             size(RhoRadGrid_C(:,:,:,2)), MPI_SUM, 0, iComm, iError)

        if (iProc == 0) call create_uniform_3d_grid_tau

        call MPI_BCAST(ExpTauRadGrid_C(:,:,:,1), &
             size(ExpTauRadGrid_C(:,:,:,1)), MPI_REAL, 0, iComm, iError)
        call MPI_BCAST(ExpTauRadGrid_C(:,:,:,2), &
             size(ExpTauRadGrid_C(:,:,:,2)), MPI_REAL, 0, iComm, iError)

        ! Reseting for next timestep
        RhoRadGrid_C(:,:,:,1:2) = 0.0
      endif
...
!==============================================================================
  subroutine create_uniform_3d_grid_rho(iBlock)
    ...
    use ModVarIndexes,  ONLY: H1Rho_, He4Rho_
    use ModPhysics,     ONLY: rBody, BodyRho_I
    ...
    do k = iMinZ,iMaxZ; do j = iMinY,iMaxY; do i = iMinX,iMaxX
      ...
      RhoRadGrid_C(i,j,k,1) = &
        interpolate_scalar(State_VGB(H1Rho_,:,:,:,iBlock),                   &
                           ...
                           DoExtrapolate)
      RhoRadGrid_C(i,j,k,2) = &
        interpolate_scalar(State_VGB(He4Rho_,:,:,:,iBlock),                  &
                           3, [ MinI, MinJ, MinK ], [ MaxI, MaxJ, MaxK ],    &
                           [ RadGridX_C(i), RadGridY_C(j), RadGridZ_C(k) ],  &
                           Xyz_DGB(x_,MinI:MaxI,1        ,1        ,iBlock), &
                           Xyz_DGB(y_,1        ,MinJ:MaxJ,1        ,iBlock), &
...
!==============================================================================
  subroutine create_uniform_3d_grid_tau
    ...
    use ModVarIndexes, ONLY: MassFluid_I
    use ModPhysics,    ONLY: No2Si_V, No2Io_V, UnitX_, UnitN_, rBody

    implicit none

    ! Local variables
    integer :: idum
    real    :: CellSizeX, TauRadGrid_C(nRadGridX, nRadGridY, nRadGridZ, 2)

  	! Summing density in all cells along x to get optical depth
    ! This is zero in the first point, at the left edge of the grid
    TauRadGrid_C(1,:,:,1:2) = RhoRadGrid_C(1,:,:,1:2) &
         * (RadGridX_C(2) - RadGridX_C(1))

  	do idum = 2,nRadGridX
      CellSizeX = RadGridX_C(idum) - RadGridX_C(idum-1)
      TauRadGrid_C(idum,:,:,1:2) = RhoRadGrid_C(idum,:,:,1:2) * CellSizeX &
           + TauRadGrid_C(idum-1,:,:,1:2)
  	enddo

    ! Normalisation of optical depth
    TauRadGrid_C(:,:,:,1) = TauRadGrid_C(:,:,:,1) * TauCoefH1
    TauRadGrid_C(:,:,:,2) = TauRadGrid_C(:,:,:,2) * TauCoefHe4

    ExpTauRadGrid_C(:,:,:,1:2) = exp(-TauRadGrid_C(:,:,:,1:2))
...
!==============================================================================
  subroutine print_model_info
    ...
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyRho fluid 1 = ', BodyRho_I(iHp_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyRho fluid 2 = ', BodyRho_I(iHep_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyRho fluid 3 = ', BodyRho_I(iH1_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyRho fluid 4 = ', BodyRho_I(iHe4_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyNDim fluid 1 = ', BodyNDim_I(iHp_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyNDim fluid 2 = ', BodyNDim_I(iHep_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyNDim fluid 3 = ', BodyNDim_I(iH1_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'BodyNDim fluid 4 = ', BodyNDim_I(iHe4_),' '
    write(*,'(5X,A23,es10.3,A10)') &
         'FluidMass 1 = ', MassFluid_I(iHp_),'  '
    write(*,'(5X,A23,es10.3,A10)') &
         'FluidMass 2 = ', MassFluid_I(iHep_),'  '
    write(*,'(5X,A23,es10.3,A10)') &
         'FluidMass 3 = ', MassFluid_I(iH1_),'  '
    write(*,'(5X,A23,es10.3,A10)') &
         'FluidMass 4 = ', MassFluid_I(iHe4_),'  '
