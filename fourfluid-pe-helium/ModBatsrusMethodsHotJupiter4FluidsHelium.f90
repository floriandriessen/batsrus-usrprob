...
contains
  ...
  contains
    ...
    subroutine set_initial_conditions
      ...
      if(restart)then
         call user_action('reading restart files') !AAV
  ...
  subroutine BATS_advance(TimeSimulationLimit)
    ...
    use ModUser, ONLY: create_uniform_3d_grid_tau, RhoRadGrid_C,  &
        ExpTauRadGrid_C !AAV
    ...
    if(nProc > 1) then
    	...
    	call MPI_reduce_real_array(RhoRadGrid_C(:,:,:,1), size(RhoRadGrid_C(:,:,:,1)),&
    	     MPI_SUM, 0, iComm, iError)
      call MPI_reduce_real_array(RhoRadGrid_C(:,:,:,2), size(RhoRadGrid_C(:,:,:,2)),&
           MPI_SUM, 0, iComm, iError)
    	if (iproc ==0) call create_uniform_3d_grid_tau
		!SC:Send tau to all processors
		  call MPI_Bcast(ExpTauRadGrid_C(:,:,:,1), size(ExpTauRadGrid_C(:,:,:,1)), MPI_REAL, 0, iComm, iError)
      call MPI_Bcast(ExpTauRadGrid_C(:,:,:,2), size(ExpTauRadGrid_C(:,:,:,2)), MPI_REAL, 0, iComm, iError)
    	!call MPI_BARRIER(iComm,iError)  !AAV
    	RhoRadGrid_C(:,:,:,1:2) = 0.0! reseting for next timestep
