...
contains
  ...
  subroutine write_tecplot_auxdata(iUnitIn)
    ...
    use ModPhysics, ONLY : &
         ThetaTilt, Rbody, ClightFactor, BodyNDim_I, Gamma_I, rPlanetSi
    ...
    ! Flo: extra
    use ModConst,      ONLY: cGravitation, cAU, mSun
    use ModUser,       ONLY: OmegaOrbitSi, RorbitAu
    ...
    ! BODYDENSITY
    ! Flo: store the numberdensity of each fluid instead of only first fluid
    write(stmp,'(es12.2)') BodyNDim_I(1)
    write(iUnitHere,'(a,a,a)') &
      'AUXDATA BODYNUMDENSITY1="',trim(adjustl(stmp)),'"'

    write(stmp,'(es12.2)') BodyNDim_I(2)
    write(iUnitHere,'(a,a,a)') &
      'AUXDATA BODYNUMDENSITY2="',trim(adjustl(stmp)),'"'
    ...
    ! Flo: here are original AAV extras
    ! rPlanetSi in cm
    write(stmp,'(es12.2)') rPlanetSi * 100.
    write(iUnitHere,'(a,a,a)') 'AUXDATA Rp_cm="',trim(adjustl(stmp)),'"'

    ! OmegaOrbit in seconds
    write(stmp,'(es12.2)') OmegaOrbitSi
    write(iUnitHere,'(a,a,a)') 'AUXDATA OmegaOrb_sec="',trim(adjustl(stmp)),'"'

    ! RorbitAu
    write(stmp,'(f14.6)') RorbitAu
    write(iUnitHere,'(a,a,a)') 'AUXDATA Rorbit_au="',trim(adjustl(stmp)),'"'

    ! Mstar in kg
    write(stmp,'(f14.6)')  OmegaOrbitSi**2 * (RorbitAu*cAU)**3 / (cGravitation* mSun)
    write(iUnitHere,'(a,a,a)') 'AUXDATA Mstar="',trim(adjustl(stmp)),'"'
